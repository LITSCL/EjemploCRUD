...
if (isset($_GET["columnaCategoria"])) {
    ...
    $productos = $daoProducto->findFK($_GET["columnaCategoria"], $_GET["id"]);
}
else {
    $daoProducto = new ProductoDAO();
    $productos = $daoProducto->findFK(false, 1);
}
?>
...
<body>
	...
	<h1>FindFK</h1>
	<h3>Permite buscar un registro en base a su clave primaria, pero cambiando la clave foranea (Mostrar otra columna de la tabla agena, en lugar de su clave primaria), si no se entregan parámetros al método se muestran las claves primarias por defecto</h3>
    
    <form action="buscar_producto_alterando_la_fk.php" method="GET">
